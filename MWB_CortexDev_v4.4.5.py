...
class Api:
    def __init__(self):
        # In-memory log buffer (keeps last 400 entries to be more terminal-like)
        self._log_buffer = []
        ...
        # reduce false positives by throttling long-running tasks
        self._task_locks = {}
        ...
        # A small trusted-signers whitelist to help classify "safe" apps (improve as needed)
    ...
    def _log(self, msg, level="INFO", details=None):
        ...
        try:
            ...
            with self._log_lock:
                ...
                # keep more entries for better terminal-like history
                if len(self._log_buffer) > 400:
    ...
    def _download_with_progress(self, url, dest_path, callback_js_name=None, download_id=None, retries=2, timeout=60):
        """Download file in chunks and report progress via JS callback (if available). Improved resilience and retry."""
        attempt = 0
        last_err = None
        while attempt <= retries:
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "MWB-Updater/1.0"})
                with urllib.request.urlopen(req, timeout=timeout) as resp:
                    ...
                    chunk_size = 65536  # larger chunks for speed
                    read = 0
                    start = time.time()
                    last_update = start
                ...
                last_err = str(e)
                attempt += 1
                self._log(f"Download attempt {attempt} failed for {url}: {last_err}", "WARN")
    ...
    def run_install(self, uid):
        """Download installer inside the app (no winget/powershell) and show download UI with progress, speed, ETA.
           Improved stability and more robust error handling."""
        def task():
            ...
            # prevent running multiple installs for same uid simultaneously
            if self._task_locks.get(f"install_{uid}"):
                self._log(f"Install already running for {uid}", "WARN")
                try:
                    webview.active_window().evaluate_js(f"showToast('Install already running for {info.get('name')}', 'info')")
                except: pass
                return
            self._task_locks[f"install_{uid}"] = True
            ...
            try:
                # instruct JS to show download overlay with improved (epic) animation
                win = webview.active_window()
                if win:
                    try:
                ...
                self._task_locks.pop(f"install_{uid}", None)
    ...
    # --- Unsafe apps scanner (BETA) ---
    def _sha256_of_file(self, path, max_read_mb=64):
        """Compute SHA256 for file; read in chunks and avoid reading enormous files into memory.
           Return hex digest or None on error."""
        try:
            h = hashlib.sha256()
            ...
            self._log(f"SHA calc failed for {path}: {str(e)}", "WARN")
            return None

    def _get_installed_apps_registry(self):
        """Collect installed applications from registry Uninstall keys (HKLM and HKCU)."""
        apps = []
        if winreg is None:
            return apps
        roots = [
                ...
                while True:
                    ...
                    try:
                        ...
                        try:
                            ...
                            install_loc = None
                        try:
                            uninstall = winreg.QueryValueEx(skey, "UninstallString")[0]
                        except Exception:
                            uninstall = None
                        apps.append({
                            "name": display,
    ...
    def _find_executables_for_app(self, app_entry):
        """Attempt to find main executables for an app using heuristic:
           - If uninstall string contains path to exe -> use it
           - If install_location provided -> scan for largest exe in that folder
           - fallback: search ProgramFiles for folder name match"""
        results = []
        try:
            uninstall = app_entry.get("uninstall_string") or ""
            install_loc = app_entry.get("install_location") or ""
            name = app_entry.get("name") or ""
            # check uninstall string for exe path
            m = re.search(r'(["\']?)([A-Za-z]:\\\\[^"\']+?\.exe)\1', uninstall)
            if m:
                path = m.group(2)
                if os.path.exists(path):
                    results.append(path)
                    return results
            # if install location, find largest exe file (heuristic for main exe)
                    ...
                    for f in files:
                        if f.lower().endswith(".exe"):
                            ...
                            try:
                                ...
                                size = 0
                            exes.append((size, full))
                if exes:
                    exes.sort(reverse=True)
                    # return top 1-3 executables
                    results.extend([p for _, p in exes[:3]])
                    return results
            # fallback: search common Program Files folders for folder name
            pf = os.environ.get("ProgramFiles", r"C:\Program Files")
            pf86 = os.environ.get("ProgramFiles(x86)", r"C:\Program Files (x86)")
            candidates = [pf, pf86, os.path.join(os.environ.get("USERPROFILE", ""), "AppData", "Local", "Programs")]
            pattern = re.sub(r'[^A-Za-z0-9]', '.*', name)[:60]
            ...
            self._log(f"Exe search failed for {app_entry.get('name')}: {str(e)}", "WARN")
            return results

    def _authenticode_signature_info(self, file_path):
        """Use PowerShell Get-AuthenticodeSignature to detect signature validity and signer (best-effort)."""
        try:
            if not os.path.exists(file_path):
                return {"status": "NotFound", "signer": None}
            # Build safe PowerShell command; use -ErrorAction SilentlyContinue
            ps = f"try{{ $s = Get-AuthenticodeSignature -FilePath '{file_path}' -ErrorAction SilentlyContinue; if($s -ne $null) {{ $out = [PSCustomObject]@{{Status = $s.Status.ToString(); Signer = ($s.SignerCertificate.Subject -replace '^[^,]+,\\s*','') }}; $out | ConvertTo-Json -Compress }} else {{ '{{\"Status\":\"Unknown\",\"Signer\":null}}' }} }} catch {{ '{{\"Status\":\"Error\",\"Signer\":null}}' }}"
            ...
            try:
                ...
                status = parsed.get("Status", parsed.get("status", "Unknown"))
                signer = parsed.get("Signer") or parsed.get("signer") or None
                return {"status": status, "signer": signer}
            except Exception:
                # sometimes PS outputs single-line objects; try to interpret
                return {"status": out, "signer": None}
        except Exception as e:
            self._log(f"Signature check failed for {file_path}: {str(e)}", "WARN")
            return {"status": "Error", "signer": None}

    def check_unsafe_apps(self):
        """Runs a detailed scan of installed apps, computes SHA256 and checks signatures (BETA).
           This runs in a background thread and sends results to UI when done."""
        def task():
            uid = f"unsafe_scan_{int(time.time())}"
            if self._task_locks.get(uid):
                self._log("Unsafe apps scan already running", "WARN")
                return
            self._task_locks[uid] = True
            try:
                self._log("Starting Unsafe Apps scan (BETA)...", "INFO")
                apps = self._get_installed_apps_registry()
                results = []
                count = 0
                for app in apps:
                    # progress update to UI every N items
                    count += 1
                    name = app.get("name") or "Unknown"
                    try:
                        exes = self._find_executables_for_app(app)
                        if not exes:
                            results.append({
                                "app": name,
                                "paths": [],
                        ...
                        app_entries = []
                        for p in exes:
                            sha = self._sha256_of_file(p)
                            sig = self._authenticode_signature_info(p)
                            signer = sig.get("signer")
                            status = sig.get("status")
                            signed_flag = (status and status.lower() == "valid")
                            # classify safety:
                            safety = "unknown"
                            if signed_flag:
                                # consider trusted if signer contains any trusted string
                                if signer:
                                    for t in self._trusted_signers:
                                        if t.lower() in signer.lower():
                                            safety = "safe"
                                            break
                                if safety != "safe":
                                    safety = "signed_but_untrusted"
                            else:
                                # unsigned binary - caution
                                safety = "unsigned"
                            app_entries.append({
                                "path": p,
                                "sha256": sha,
                        ...
                        # aggregate best safety for app
                        final_safety = "unknown"
                        if any(e["safety"] == "unsigned" for e in app_entries):
                            final_safety = "unsigned"
                        elif any(e["safety"] == "signed_but_untrusted" for e in app_entries):
                            final_safety = "signed_but_untrusted"
                        elif any(e["safety"] == "safe" for e in app_entries):
                            final_safety = "safe"
                        results.append({
                            "app": name,
                            "exes": app_entries,
                        ...
                        self._log(f"Error analyzing {name}: {str(e)}", "ERROR")
                        results.append({
                            "app": name,
                            "exes": [],
                            "status": "error"
                        })
                    # send periodic progress updates to UI (non-blocking)
                    try:
                        win = webview.active_window()
                        if win:
                            payload = {"processed": count, "total": len(apps)}
                            win.evaluate_js(f"updateUnsafeScanProgress({json.dumps(payload)})")
                    except:
                        pass
                # final result -> send results to UI in compressed form
                try:
                    win = webview.active_window()
                    if win:
                        safe = json.dumps(results)
                        # send results to create a modal with animation
                        win.evaluate_js(f"createUnsafeScanModal({safe})")
                        win.evaluate_js("showToast('Unsafe Apps Scan completed (BETA)', 'success')")
                except Exception as e:
                    self._log(f"Failed to send scan results to UI: {str(e)}", "ERROR")
                self._log("Unsafe Apps scan finished.", "INFO")
            except Exception as e:
                self._log(f"Unsafe Apps scan failed: {str(e)}", "ERROR")
            finally:
                self._task_locks.pop(uid, None)

        threading.Thread(target=task, daemon=True).start()
        return "Unsafe scan started"

    # --- Updater (ALPHA) ---
    def _semver_compare(self, a, b):
        """
        Compare semantic versions a and b.
        Return 1 if a > b, 0 if equal, -1 if a < b
        Accepts strings like 'v1.2.3' or '1.2.3'
        """
        def norm(s):
            s = str(s).strip()
            s = s.lstrip('vV')
    ...
    def _fetch_latest_release(self, repo="CortexDev-Official/Make-Windows-Better", timeout=15):
        """
        ...
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "MWB-Updater/1.0", "Accept": "application/vnd.github.v3+json"})
            with urllib.request.urlopen(req, timeout=timeout) as resp:
    ...
    def check_for_updates(self):
        ...
        - If a newer version exists, downloads suitable asset (.exe or .msi)
        - Runs the downloaded installer elevated and exits current app
        This operation is marked ALPHA in UI. Very cautious with retries and validation.
        """
        def task():
            uid = f"update_check_{int(time.time())}"
            if self._task_locks.get(uid):
                self._log("Update check already running", "WARN")
                return
            self._task_locks[uid] = True
            try:
                self._log("Checking for updates (ALPHA)...", "INFO")
                win = webview.active_window()
                if win:
                    try:
                        win.evaluate_js("showToast('Checking for updates...', 'info')")
                    except: pass

                meta, err = self._fetch_latest_release()
                if err or not meta:
                    self._log(f"Failed to fetch release metadata: {err}", "ERROR")
                    if win:
                        try:
                            win.evaluate_js("showToast('Update check failed', 'error')")
                        except: pass
                    return

                tag = meta.get("tag_name") or meta.get("name") or ""
                remote_version = str(tag).lstrip('vV')
                self._log(f"Latest release tag: {tag}", "INFO")
                cmp = self._semver_compare(remote_version, VERSION)
                if cmp <= 0:
                    self._log("No update available", "INFO")
                    if win:
                        try:
                            win.evaluate_js("showToast('No updates available', 'info')")
                        except: pass
                    return

                # find best asset (prefer .exe, then .msi)
                ...
                if not download_asset and assets:
                    for a in assets:
                ...
                if not download_asset:
                    self._log("No suitable asset found in release", "ERROR")
                    if win:
                        try:
                            win.evaluate_js("showToast('Update found but no installer asset available', 'error')")
                        except: pass
                    return

                url = download_asset.get("browser_download_url")
                name = download_asset.get("name") or f"mwb_update_{remote_version}"
                self._log(f"Selected asset for download: {name} ({url})", "INFO")

                tempdir = os.path.join(tempfile.gettempdir(), "mwb_updates")
                os.makedirs(tempdir, exist_ok=True)
                download_id = str(uuid.uuid4())
                ...
                if win:
                    try:
                        win.evaluate_js(f"showDownloadOverlay({json.dumps(download_id)}, {json.dumps('MWB Update ' + remote_version)})")
                    except: pass

                ok, derr = self._download_with_progress(url, dest, callback_js_name="updateDownloadProgress", download_id=download_id, retries=3, timeout=120)
                if not ok:
                    self._log(f"Update download failed: {derr}", "ERROR")
                    if win:
                        try:
                            win.evaluate_js("showToast('Update download failed', 'error')")
                            win.evaluate_js(f"hideDownloadOverlay({json.dumps(download_id)})")
                        except: pass
                    return

                # basic validation: ensure file exists and non-zero size
                try:
                    if not os.path.exists(dest) or os.path.getsize(dest) < 1024:
                        self._log("Downloaded update seems invalid (missing or too small)", "ERROR")
                        if win:
                            try:
                                win.evaluate_js("showToast('Downloaded update seems invalid', 'error')")
                                win.evaluate_js(f"hideDownloadOverlay({json.dumps(download_id)})")
                            except: pass
                        return
                except Exception as e:
                    ...
                    if win:
                        try:
                            win.evaluate_js("showToast('Validation failed for update', 'error')")
                            win.evaluate_js(f"hideDownloadOverlay({json.dumps(download_id)})")
                        except: pass
                    return

                # Attempt to run installer elevated. Be cautious: give user info in logs.
                try:
                    self._log(f"Launching update installer: {dest}", "INFO")
                    # If MSI use msiexec
                    if dest.lower().endswith(".msi"):
                        # start msiexec asynchronously
                        subprocess.Popen(["msiexec", "/i", dest], shell=False, creationflags=CREATE_NO_WINDOW)
                    else:
                        # run elevated; show hidden window; allow user to cancel UAC
                        ctypes.windll.shell32.ShellExecuteW(None, "runas", dest, "", None, SW_HIDE)
                    if win:
                        try:
                            win.evaluate_js("showToast('Update installer launched. Closing old version...', 'success')")
                        except: pass
                    # close current UI gracefully
                    try:
                    ...
                    self._log(f"Failed to launch update installer: {str(e)}", "ERROR")
                    if win:
                        try:
                            win.evaluate_js("showToast('Failed to start installer', 'error')")
                            win.evaluate_js(f"hideDownloadOverlay({json.dumps(download_id)})")
                        except: pass
                    return

            except Exception as e:
                self._log(f"Update check failed: {str(e)}", "ERROR")
            finally:
                self._task_locks.pop(uid, None)
...
# HTML_UI is similar to previous versions but updated to v4.4.5 and with the new 'Check For Update (ALPHA)' button alongside Unsafe Apps.
# Due to length of UI code, it's included verbatim below with small modifications (version texts, update button & JS hook).
HTML_UI = r"""
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { -webkit-tap-highlight-color: transparent; outline: none; scroll-behavior: smooth; cursor: default; }
        ...
        /* Download overlay: upgraded "epic" entry animation */
        #download-overlay {
            position: fixed;
            inset: 0;
            display: none;
        ...
        #download-overlay.show {
            display: flex;
            pointer-events: auto;
            animation: downloadEnter 0.9s cubic-bezier(0.2,0.9,0.2,1) forwards;
        }
        @keyframes downloadEnter {
            0% { opacity: 0; transform: translateY(24px) scale(0.92) rotate(-1deg); filter: blur(8px); }
            60% { opacity: 1; transform: translateY(-8px) scale(1.03) rotate(0.4deg); filter: blur(0px); }
        ...
        .download-bar > div {
            height: 100%;
            width: 0%;
            background: linear-gradient(90deg, #00ffd5, #3b82f6 60%, #7c3aed 100%);
        ...
        #unsafe-modal.show {
            display: flex;
            animation: modalEntrance 0.7s cubic-bezier(0.2,0.9,0.2,1) forwards;
        }
        ...
        .unsafe-item.safe { border-left: 4px solid #22c55e; }
        .unsafe-item.unsigned { border-left: 4px solid #ef4444; }
        .unsafe-item.signed_but_untrusted { border-left: 4px solid #f59e0b; }
        ...
        /* small ALPHA badge */
        .alpha-badge { color: white; background: #b91c1c; padding: 4px 8px; border-radius: 8px; font-weight: 800; font-size: 11px; }
    </style>
</head>
<body>
        ...
        <div class="welcome-container ray-trace-enabled">
            <div class="welcome-sidebar">
                ...
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-relaxed">
                    Personalized System Optimization <br> Version 4.4.5
                </div>
            </div>
            ...
            <div class="welcome-content">
                ...
                <div id="w-stage-2" class="welcome-stage">
                    ...
                    <ul class="space-y-5 mb-10">
                        ...
                        <li class="flex items-center gap-4 text-xs font-bold text-gray-300">
                            <span class="w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_10px_#3b82f6]"></span> 4.4.5 KERNEL BOOST
                        </li>
                    </ul>
        ...
        <div id="page-dashboard" class="page active">
            ...
            <div class="grid grid-cols-3 gap-8 mb-10">
                ...
                <div class="card border-orange-500/20 bg-orange-500/5 ray-trace-enabled">
                    ...
                    <h3 class="text-3xl font-black text-orange-500 italic">v4.4.5</h3>
            ...
            <div class="grid grid-cols-3 gap-8 mt-8">
                ...
                <div class="card ray-trace-enabled">
                    <p class="text-[10px] font-black text-gray-500 mb-3 uppercase tracking-widest">Security</p>
                    <h3 class="text-2xl font-black italic mb-6">Check Unsafe Apps <span class="text-xs text-yellow-300">BETA</span></h3>
                    <div class="flex gap-2">
                      <button onclick="checkUnsafeApps()" class="install-btn !bg-yellow-600 hover:!bg-yellow-500 w-full ray-trace-enabled">Check Unsafe Apps (BETA)</button>
                      <button onclick="checkForUpdate()" class="install-btn !bg-red-700 hover:!bg-red-600 w-full ray-trace-enabled">Check For Update <span style="margin-left:8px" class="alpha-badge">ALPHA</span></button>
                    </div>
                </div>
            </div>
        ...
        <div id="page-about" class="page text-center">
            <div class="py-32 flex flex-col items-center">
                ...
                <div class="flex space-x-6">
                    <span class="bg-white/5 px-10 py-3 rounded-full text-[11px] font-black border border-white/10 uppercase tracking-widest hover:bg-white/10 transition-colors cursor-default">Build v4.4.5</span>
    ...
    <!-- Unsafe scan modal -->
    <div id="unsafe-modal">
        <div class="unsafe-card" id="unsafe-card">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <div class="text-xl font-bold">Unsafe Apps Scan Results (BETA)</div>
                    <div class="text-xs text-gray-400">Results are best-effort. Signed apps from well-known vendors are suggested safe.</div>
                </div>
                <div>
                    <button onclick="closeUnsafeModal()" class="install-btn !bg-gray-700">Close</button>
                </div>
            </div>
            <div id="unsafe-results" class="space-y-3"></div>
        </div>
    </div>
    ...
    <script>
        ...
        // Download overlay controls with epic animation
        function showDownloadOverlay(id, name) {
            ...
            // use class to trigger CSS animation
        ...
        // Unsafe scanner UI helpers
        function updateUnsafeScanProgress(payload) {
            // simple toast-style progress so user isn't disturbed
            if(payload && payload.total) {
                const pct = Math.round((payload.processed / payload.total) * 100);
                showToast(`Unsafe Scan: ${payload.processed}/${payload.total} (${pct}%)`, 'info');
            }
        }

        function createUnsafeScanModal(results) {
            try {
                const modal = document.getElementById('unsafe-modal');
                const container = document.getElementById('unsafe-results');
                container.innerHTML = '';
                // sort by severity: unsigned -> signed_but_untrusted -> unknown -> safe
                results.sort((a,b)=>{
                    const rank = { "unsigned":0, "signed_but_untrusted":1, "unknown":2, "safe":3, "error":4, "no_exe_found":5 };
                    return (rank[a.status]||9) - (rank[b.status]||9);
                });
                results.forEach(r=>{
                    const item = document.createElement('div');
                    const status = r.status || 'unknown';
                    const cls = status === 'safe' ? 'safe' : status === 'unsigned' ? 'unsigned' : (status === 'signed_but_untrusted' ? 'signed_but_untrusted' : '');
                    item.className = `unsafe-item ${cls} p-3 rounded-md bg-white/2`;
                    let inner = `<div class="flex justify-between"><div><div style="font-weight:800">${r.app}</div><div style="font-size:12px;color:#bfc7d6;margin-top:6px">Status: ${status}</div></div>`;
                    inner += `<div style="text-align:right">`;
                    if(r.exes && r.exes.length){
                        inner += `<div style="font-size:12px;color:#9aa8bf">Executables:</div>`;
                        r.exes.forEach(e=>{
                            inner += `<div style="font-size:12px;margin-top:4px">${e.path}<br/><span style="color:#9aa8bf">SHA256:</span> ${e.sha256?e.sha256.substring(0,16)+'...': 'N/A'} <span style="color:#9aa8bf">Signer:</span> ${e.signer||'N/A'}</div>`;
                        });
                    } else {
                        inner += `<div style="font-size:12px;color:#9aa8bf">No executables found</div>`;
                    }
                    inner += `</div></div>`;
                ...
                showToast("Failed to render scan results", "error");
            }
        }
        ...
        // New: Check Unsafe Apps (BETA)
        function checkUnsafeApps() {
            try {
                showToast("Starting Unsafe Apps scan (BETA)...", "info");
                if(window.pywebview && window.pywebview.api) {
                    window.pywebview.api.check_unsafe_apps();
                }
            } catch(e) {
                showToast("Failed to start Unsafe Apps scan", "error");
            }
        }

        // New: Check For Update (ALPHA)
        function checkForUpdate() {
            try {
                showToast("Checking for updates (ALPHA)...", "info");
                if(window.pywebview && window.pywebview.api) {
                    window.pywebview.api.check_for_updates();
                }
            } catch(e) {
                showToast("Update check failed to start", "error");
            }
        }
        ...
        window.addEventListener("pywebviewready", () => {
            ...
            showToast("MWB v4.4.5 Loaded", "success");
        });
    </script>
