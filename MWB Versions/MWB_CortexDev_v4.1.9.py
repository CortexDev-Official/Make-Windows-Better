...
class Api:
    ...
    # --- New App Manager Uninstaller Function ---
    def uninstall_sys_app(self, package_name):
        def task():
            cmd = f'powershell -Command "Get-AppxPackage *{package_name}* | Remove-AppxPackage"'
            subprocess.run(cmd, shell=True)
            webview.active_window().evaluate_js(f"showToast('Attempted to remove {package_name}')")
        threading.Thread(target=task, daemon=True).start()
        return "Uninstalling..."
...
<head>
    ...
    <style>
        :root {
            ...
            --ray-reflect: linear-gradient(135deg, rgba(255,255,255,0.15) 0%, rgba(255,255,255,0) 50%, rgba(255,255,255,0.05) 100%);
        }
        ...
        * { -webkit-tap-highlight-color: transparent; outline: none; scroll-behavior: smooth; }
        ...
        /* Ray Tracing Reflective Effect */
        .card, .sidebar-item, .action-btn, .install-btn {
            position: relative;
            overflow: hidden;
        }
        .card::after, .action-btn::after {
            content: '';
            position: absolute;
            inset: 0;
        ...
        .step-dot.active {
            ...
            box-shadow: 0 0 15px var(--dynamic-glow);
        }

        .welcome-stage {
            display: none;
            animation: stageFade 0.8s cubic-bezier(0.23, 1, 0.32, 1) forwards;
        }

        .welcome-stage.active { display: block; }
        ...
        .action-btn:hover {
            ...
            box-shadow: 0 15px 30px var(--dynamic-glow);
        ...
        .sidebar-item.active { 
            ...
            color: var(--accent);
            box-shadow: 0 4px 15px rgba(0,0,0,0.2), 0 0 20px rgba(0, 122, 255, 0.1);
        }
        ...
        .card:hover {
            ...
            box-shadow: 0 20px 40px rgba(0,0,0,0.4), 0 0 30px rgba(0, 122, 255, 0.05);
        }
...
<body>
    ...
    <div id="welcome-overlay">
        <div class="welcome-container">
            <div class="welcome-sidebar">
                <div>
                    <h1 class="text-4xl font-black italic tracking-tighter text-blue-500 mb-2">MWB</h1>
                    <p class="text-[9px] font-bold uppercase tracking-[0.4em] opacity-50">CortexDev Official</p>
                </div>
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-relaxed">
                    Personalized System Optimization <br> Build v4.1.9 Stable
                </div>
            </div>
            ...
            <div class="welcome-content">
                ...
                <div id="w-stage-1" class="welcome-stage active">
                    <h2 class="text-5xl font-black italic tracking-tighter mb-4">Welcome</h2>
                    ...
                    <div class="card bg-blue-500/[0.03] border-blue-500/20 mb-8 p-5">
                        <p class="text-[11px] font-black uppercase text-blue-400 tracking-widest mb-3">Community Support</p>
                        <p class="text-xs text-gray-500 mb-3 italic">Help us grow by starring our project on GitHub!</p>
                    ...
                    <button onclick="nextStep(2)" class="action-btn">Continue to Mission</button>
                </div>

                <div id="w-stage-2" class="welcome-stage">
                    <h2 class="text-5xl font-black italic tracking-tighter mb-4">Our Vision</h2>
                    <p class="text-gray-400 text-sm leading-relaxed mb-8">
                    ...
                    <ul class="space-y-4 mb-8">
                        <li class="flex items-center gap-3 text-xs font-bold text-gray-300">
                            <span class="text-blue-500">◈</span> JUNK & BLOATWARE REMOVAL
                        </li>
                        <li class="flex items-center gap-3 text-xs font-bold text-gray-300">
                            <span class="text-blue-500">◈</span> TELEMETRY DEACTIVATION
                        </li>
                        <li class="flex items-center gap-3 text-xs font-bold text-gray-300">
                            <span class="text-blue-500">◈</span> KERNEL LATENCY REDUCTION
                        </li>
                        <li class="flex items-center gap-3 text-xs font-bold text-gray-300">
                            <span class="text-blue-500">◈</span> AND MUCH MORE...
                        </li>
                    </ul>
                    <button onclick="nextStep(3)" class="action-btn">Finalize Setup</button>
                </div>

                <div id="w-stage-3" class="welcome-stage">
                    <h2 class="text-5xl font-black italic tracking-tighter mb-4">All Set</h2>
                    <p class="text-gray-400 text-sm leading-relaxed mb-8">
                    ...
                    <div class="h-24 flex items-center justify-center bg-white/5 rounded-3xl mb-8">
                        <div class="flex gap-4">
                            <div class="w-2 h-2 bg-blue-500 rounded-full animate-ping"></div>
                            <div class="w-2 h-2 bg-blue-500 rounded-full animate-ping delay-75"></div>
                            <div class="w-2 h-2 bg-blue-500 rounded-full animate-ping delay-150"></div>
                        </div>
                    </div>
                    <button onclick="finishWelcome()" class="action-btn !bg-white !text-black hover:!bg-blue-500 hover:!text-white">Launch Dashboard</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="w-80 h-full liquid-sidebar flex flex-col py-12 z-50">
        ...
        <nav id="nav-list" class="flex-1">
            ...
            <div onclick="switchPage('appmanager', this)" class="sidebar-item">
    ...
    <div class="page-container" id="page-container">
        ...
        <div id="page-appmanager" class="page">
            <h2 class="text-7xl font-black mb-4 tracking-tighter italic">App Manager</h2>
            <p class="text-red-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
                The developer, CortexDev, is not responsible for any harm or damage to your computer
        ...
        <div id="page-about" class="page text-center">
            <div class="py-32 flex flex-col items-center">
                ...
                <div class="flex space-x-6">
                    <span class="bg-white/5 px-10 py-3 rounded-full text-[11px] font-black border border-white/10 uppercase tracking-widest">Build v4.1.9</span>
    ...
    <script>
        ...
        function finishWelcome() {
            ...
            overlay.style.transform = 'translateY(-50px) scale(0.95)';
        ...
        // --- New System App Management List (20+ Apps) ---
        const sysApps = [
            { pkg: 'Microsoft.BingNews', name: 'MS News' },
            { pkg: 'Microsoft.BingWeather', name: 'MS Weather' },
            { pkg: 'Microsoft.ZuneVideo', name: 'Movies & TV' },
            { pkg: 'Microsoft.ZuneMusic', name: 'Groove Music' },
            { pkg: 'Microsoft.GetHelp', name: 'Get Help' },
            { pkg: 'Microsoft.Getstarted', name: 'Tips' },
            { pkg: 'Microsoft.Messaging', name: 'Messaging' },
            { pkg: 'Microsoft.MicrosoftSolitaireCollection', name: 'Solitaire' },
            { pkg: 'Microsoft.OneConnect', name: 'Paid Wi-Fi' },
            { pkg: 'Microsoft.People', name: 'People' },
            { pkg: 'Microsoft.SkypeApp', name: 'Skype' },
            { pkg: 'Microsoft.WindowsAlarms', name: 'Alarms' },
            { pkg: 'Microsoft.WindowsCamera', name: 'Camera' },
            { pkg: 'Microsoft.WindowsFeedbackHub', name: 'Feedback Hub' },
            { pkg: 'Microsoft.WindowsMaps', name: 'Maps' },
            { pkg: 'Microsoft.WindowsSoundRecorder', name: 'Voice Recorder' },
            { pkg: 'Microsoft.YourPhone', name: 'Your Phone' },
            { pkg: 'Microsoft.XboxApp', name: 'Xbox App' },
            { pkg: 'Microsoft.XboxGameOverlay', name: 'Xbox Overlay' },
            { pkg: 'Microsoft.XboxGamingOverlay', name: 'Xbox Game Bar' },
            { pkg: 'Microsoft.XboxSpeechToTextOverlay', name: 'Xbox Text-to-Speech' },
            { pkg: 'Microsoft.MicrosoftStickyNotes', name: 'Sticky Notes' }
        ];
        ...
        apps.forEach(app => {
            softGrid.innerHTML += `
                <div class="card group">
                    <h3 class="text-lg font-black text-white italic mb-1">${app.name}</h3>
                    <p class="text-[10px] text-gray-500 mb-6 uppercase tracking-widest">${app.desc}</p>
                    <button onclick="pywebview.api.run_install('${app.id}')" class="install-btn w-full">Deploy</button>
                </div>`;
        });

        // Render App Manager
        const appGrid = document.getElementById('appmanager-grid');
        sysApps.forEach(app => {
            appGrid.innerHTML += `
                <div class="card group border-red-500/10">
                    <h3 class="text-sm font-black text-white italic mb-1">${app.name}</h3>
                    <p class="text-[9px] text-gray-500 mb-4 truncate">${app.pkg}</p>
                    <button onclick="pywebview.api.uninstall_sys_app('${app.pkg}')" class="install-btn !bg-red-600/20 !text-red-500 !border !border-red-500/30 w-full hover:!bg-red-600 hover:!text-white">Uninstall</button>
                </div>`;
        });

        // Render Optimizer
        const optGrid = document.getElementById('optimizer-grid');
        optimizers.forEach(opt => {
            optGrid.innerHTML += `
                <div onclick="pywebview.api.run_optimize('${opt.id}')" class="card flex items-center justify-between cursor-pointer hover:border-blue-500/40">
                    <div>
                        <h3 class="text-xl font-black italic text-white">${opt.name}</h3>
                        <p class="text-[10px] text-gray-500 uppercase tracking-widest mt-1">${opt.desc}</p>
                    </div>
                    <span class="text-2xl text-blue-500 opacity-20 group-hover:opacity-100 transition-opacity">⌬</span>
                </div>`;
        });
        ...
        function updateStats() {
            pywebview.api.get_stats().then(stats => {
                ...
                const thermal = document.getElementById('stat-thermal');
                if(stats.is_admin) {
                    thermal.innerText = "OPTIMAL";
                    thermal.className = "text-xl font-black text-green-500 italic";
                }
            });
        }
        ...
        function engageGaming() { 
            showToast("VELOCITY OVERRIDE ENGAGED");
