...
class Api:
    def get_stats(self):
        try:
            ...
            return {"ram": "N/A", "ram_raw": 0, "uptime": "00:00:00", "node": "Cortex-Node", "os": "Windows", "cpu": "Detecting...", "gpu": "Scanning..."}

    def run_install(self, uid):
        def task():
            # استخدام Start-Process مع -Verb RunAs لضمان الصلاحيات
    ...
    def run_optimize(self, action):
        commands = {
            ...
            "visuals": "visual_boost_internal", # placeholder for internal logic
            "gamebar": "reg add 'HKCU\\System\\GameConfigStore' /v GameDVR_Enabled /t REG_DWORD /d 0 /f"
        }
        cmd = commands.get(action)
        if cmd:
            def task():
                # تشغيل كمسؤول لتجنب "Access Denied"
    ...
    def run_gaming_boost(self):
        def task():
            # إصلاح الخطأ: تشغيل الـ Priority boost مع تجاهل أخطاء صلاحيات تطبيقات النظام
            # الـ Script الآن يحاول تغيير الأولوية فقط للتطبيقات التي يملك صلاحية عليها ولا يتوقف عند الخطأ
            ps_script = """
        ...
        <div id="page-about" class="page text-center">
            <div class="py-32 flex flex-col items-center">
                ...
                <div class="flex space-x-6">
                    <span class="bg-white/5 px-10 py-3 rounded-full text-[11px] font-black border border-white/10 uppercase tracking-widest">Build v3.0.0</span>
