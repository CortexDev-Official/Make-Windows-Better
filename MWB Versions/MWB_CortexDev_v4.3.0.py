...
class Api:
    ...
    # ==========================
    # NEW SECTION: Blackbox (Useful Features: Ghost Compress + Omni-Control)
    # ==========================
    def run_ghost_compression(self):
        def task():
            try:
                self._log("Initializing Ghost Compression (CompactOS)...", "WARN")
                # Command to compress OS binaries
                cmd = 'compact.exe /CompactOS:always'
                full_cmd = f'powershell -Command "Start-Process cmd -ArgumentList \'/c {cmd}\' -Verb RunAs"'
                subprocess.run(full_cmd, shell=True)
                self._log("Ghost Compression initiated. This runs in background.", "SUCCESS")
                webview.active_window().evaluate_js("showToast('Ghost Compression Started (Wait...)', 'info')")
        ...
        def task():
            try:
                ...
                if not os.path.exists(full_path):
                    ...
                    self._log("Omni-Control folder created on Desktop.", "SUCCESS") 
                    webview.active_window().evaluate_js("showToast('Omni-Control Created on Desktop', 'success')") 
                else:
                    self._log("Omni-Control already exists.", "INFO") 
                    webview.active_window().evaluate_js("showToast('Omni-Control already exists', 'info')") 
            except Exception as e:
...
<body>
        ...
        <div class="welcome-container ray-trace-enabled">
            <div class="welcome-sidebar">
                ...
                <div class="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-relaxed">
                    Personalized System Optimization <br> Version 4.3.0
                </div>
            </div>
            ...
            <div class="welcome-content">
                ...
                <div id="w-stage-2" class="welcome-stage">
                    ...
                    <ul class="space-y-5 mb-10">
                        ...
                        <li class="flex items-center gap-4 text-xs font-bold text-gray-300">
                            <span class="w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_10px_#3b82f6]"></span> 4.3.0 KERNEL BOOST
                        </li>
                    </ul>
        ...
        <div id="page-dashboard" class="page active">
            ...
            <div class="grid grid-cols-3 gap-8 mb-10">
                ...
                <div class="card border-orange-500/20 bg-orange-500/5 ray-trace-enabled">
                    ...
                    <h3 class="text-3xl font-black text-orange-500 italic">v4.3.0</h3>
        ...
        <div id="page-about" class="page text-center">
            <div class="py-32 flex flex-col items-center">
                ...
                <div class="flex space-x-6">
                    <span class="bg-white/5 px-10 py-3 rounded-full text-[11px] font-black border border-white/10 uppercase tracking-widest hover:bg-white/10 transition-colors cursor-default">Build v4.3.0</span>
    ...
    <script>
        ...
        // NEW CUSTOM FUNCTIONS (BLACKBOX)
        function runGhostComp() { pywebview.api.run_ghost_compression(); }
        function runGodMode() { pywebview.api.enable_god_mode(); } // This calls the renamed function
        ...
        window.addEventListener("pywebviewready", () => {
            ...
            showToast("MWB v4.3.0 Loaded", "success");
        });
    </script>
